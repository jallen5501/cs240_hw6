...
/* external specs:
just function prototypes for initalloc, alloc, and freef */
#include "alloc.h"
#include <stdio.h>
...
/*JA if (youre unchaining last element in free chain) */
    freep = (cursorp = NULL);  /* empty list results                      */
  else  {                      /* chain around p->structl                 */
    (p->prevp)->nextp = p->nextp;    /*JA Point around p */
    (p->nextp)->prevp = p->prevp;    /*JA Point around p */	 
...
/* alloc(): find a block of n bytes. 
   Actually, find what might be somewhat more, allocxize >= n bytes, guaranteed to be longword-aligned, that is, starting on an address which is a multiple of 4, and is at least enough to make up a MINFREESIZE block, long enough to enchain when free. 
   Note that a starting address multiple of 2 would be workable for numeric data on many machines (68000/68020), but a starting address multiple of 4
...
void freef(char *p) {
  ...
  /* you write the  code for the freef function here */
	...
	if (p == NULL) {
		printf("freef: NULL is not a valid argument.\n");
		return;
	}
	if (p < allocbuf || p > allocbuf + ALLOCSIZE) {
                printf("freef: Argument is not inside data structure, %p\n", (void *)p);
                return;
        }   

        struct blockl *blockl; /* beginning of new free space */
	struct blockr *blockr; /* end of new free space */
        blockl = (struct blockl *)(p - TAGSIZE); 
	blockr = (struct blockr *)(p + blockl->size - 2 * TAGSIZE); 

        if (blockl->tag != USEDTAG || blockr->tag != USEDTAG) {
		/* only want to free from blocks marked USEDTAG */
		printf("freef: cannot free an unused block, %p\n", (void *)p);
 		return;     
	}
        else /* reset the tags of buffer being free'd*/
                blockl->tag = blockr->tag = FREETAG;

        struct blockr *leftr;   /* blockr to the left of buffer being free'd */
        struct blockl *rightl;  /* blockl to the right of buffer being free'd */
        leftr = (struct blockr *)(p - 2 * TAGSIZE);
        rightl = (struct blockl *)(p + blockl->size - TAGSIZE);

        if (leftr->tag == FREETAG) { /* block to left of blockl is part of free chain  */
                struct blockl *leftl = (struct blockl *)(p - leftr->size - TAGSIZE);
		/* leftl points to blockl associated with rightl */
                coalesce(leftl, blockl); /* combine the two blocks */
		blockl = leftl; /* blockl points to new beginning of block */
	}
        else /* block to the left is not part of free chain */
		enchain(blockl);

        if (rightl->tag == FREETAG) { /* block to right of blockl is part of free chain */
                /* block to left of new block is free but block to the right is used */
                unchain(rightl); /* remove reference to rightl from the free list */
                coalesce(blockl, rightl); /* combine the two blocks */
        } /* if (rightl->tag == USEDTAG) return; */
}
